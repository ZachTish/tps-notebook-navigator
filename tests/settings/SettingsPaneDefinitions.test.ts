...
describe('settings pane definitions', () => {
    ...
    it('keeps TPS integration as the first configuration destination', () => {
        expect(SETTINGS_PAGE_GROUP_DEFINITIONS[0]?.items).toContain('tps-integration');
        expect(SETTINGS_PAGE_GROUP_DEFINITIONS[0]?.items[0]).toBe('tps-integration');
    });
});

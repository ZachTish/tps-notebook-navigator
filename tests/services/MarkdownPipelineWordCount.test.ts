...
import { ALL_TAGS_TAG_ID, ItemType, PROPERTIES_ROOT_VIRTUAL_FOLDER_ID, TAGGED_TAG_ID } from '../../src/types';
...
describe('Markdown pipeline appearance relevance', () => {
    it('uses effective upstream appearance consumers while retaining TPS Type invalidation', () => {
        ...
        expect(provider.getRelevantSettings()).toContain('typeAppearances');
    });
    ...
    it('recognizes an untagged consuming header in the TPS all-tags list', () => {
        const { app, cachedMetadataByPath, markdownFiles } = createApp();
        const headerFile = createFile('Notes/Header.md');
        markdownFiles.push(headerFile);
        cachedMetadataByPath.set(headerFile.path, {
            frontmatter: {
                group_header: { title: 'All notes', show_word_count: true }
            }
        });
        const settings = createSettings({
            textCountDisplay: 'none',
            noteGrouping: 'date',
            defaultFolderSort: 'modified-desc',
            tagAppearances: { [ALL_TAGS_TAG_ID]: { groupBy: 'custom' } },
            tagSortOverrides: { [ALL_TAGS_TAG_ID]: 'title-asc' },
            manualSortGroupHeaderProperty: 'group_header',
            includeDescendantNotes: false,
            showTags: true
        });
    ...
    it('treats a present blank property as top-level property-list membership', () => {
        const { app, cachedMetadataByPath, markdownFiles } = createApp();
        const headerFile = createFile('Notes/Header.md');
        markdownFiles.push(headerFile);
        cachedMetadataByPath.set(headerFile.path, {

...
function getTagSelectionPaths(metadata: FrontMatterCache | null, includeDescendantNotes: boolean): string[] {
    // The TPS Tags root is an aggregate all-visible-Markdown scope, so every note can contribute
    // a manual-sort group header there even when it has no authored tag.
    const paths = new Set<string>([ALL_TAGS_TAG_ID]);
    const tags = extractFileTagsFromRawTags(metadata ? getAllTags(metadata) : null);
    if (tags.length === 0) {
        paths.add(UNTAGGED_TAG_ID);
...
function getPropertySelectionPaths(frontmatter: FrontMatterCache, settings: NotebookNavigatorSettings): string[] {
    ...
    Object.entries(frontmatter).forEach(([key, value]) => {
        ...
        // A present key remains part of the property scope even when its value is blank, an empty
        // list, or another unsupported YAML shape. This mirrors the persisted property index,
        // where such values produce the top-level key's "No value" member.
        const normalizedValues = values.length === 0 ? [''] : values.map(entry => normalizePropertyTreeValuePath(entry.value));
        // TPS property-key rows are aggregate scopes: they always include every note where the
        // key exists, independent of the global descendant default.

...
- Mobile structural Type rows have a strict visual contract: they are one-line rows with the same padding, title metrics, icon-slot width, and virtual height as ordinary Notes. Source metadata is not shown on a second line on mobile. The title, checkbox, and More controls must remain transparent and shadowless even when Obsidian iOS or a theme applies generic button chrome; the checkbox is the only intentional visual substitution for the normal leading file icon. Focus, selection, task completion, and context-menu behavior remain available.

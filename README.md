...
Precedence is intentionally narrow. While **Use frontmatter metadata** is enabled, a usable authored value in the configured frontmatter field wins, followed by an explicit per-file Navigator icon or color, then the generated projection; disabling that setting continues to disable both authored and projected frontmatter-style icons/colors while leaving explicit Navigator appearances available. Generated icon values pass through the same frontmatter compatibility decoder as authored icons. Property sort and property group read authored frontmatter first and consult the projection only when that requested field has no usable value. The configured manual-sort property is never virtualized because manual ranking remains a real YAML mutation owned by Navigator.

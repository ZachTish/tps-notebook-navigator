...
- Mobile structural Type rows have a strict visual contract: they are one-line rows with the same padding, title metrics, icon-slot width, and virtual height as ordinary Notes. Source metadata is not rendered on mobile. The runtime fixes the title-row count to one and applies native-control resets inline to the title, checkbox, and More controls; CSS repeats the same constraints as progressive protection. This prevents stale or reordered iOS/theme styles from restoring gray button chrome or a second line. The checkbox is the only intentional visual substitution for the normal leading file icon. Focus, selection, task completion, and context-menu behavior remain available.

...
const SOURCE_THEME_VARIABLE_PREFIX = '--nn-theme-';
const DOCUMENTED_THEME_VARIABLE_PREFIX = '--tps-nn-theme-';

function toDocumentedThemeVariable(value) {
    return value.replace(SOURCE_THEME_VARIABLE_PREFIX, DOCUMENTED_THEME_VARIABLE_PREFIX);
}

function toSourceThemeValue(value) {
    return value.replaceAll(DOCUMENTED_THEME_VARIABLE_PREFIX, SOURCE_THEME_VARIABLE_PREFIX);
}
...
function parseThemeReference(markdown) {
    ...
    for (const line of section.split('\n')) {
        const match = /^\| `(--tps-nn-theme-[\w-]+)` \| (.*?) \| (.*?) \|$/.exec(line);
        if (!match) {
            continue;
        }
        rows.push({
...
function validateThemingSources(sourceCss, coreVariablesCss, styleSettingsCss, themingGuide) {
    // The source deliberately retains upstream `--nn-theme-*` tokens and the generated stylesheet transforms them to
    // TPS names. The guide documents those public TPS tokens, so normalize source values only for this comparison.
    ...
    const settingsVariables = settings
        ...
        .map(entry => toDocumentedThemeVariable(`--${entry.id}`));
    ...
    const sourceVariables = Array.from(
        new Set((sourceWithoutComments.match(/--nn-theme-[\w-]+/g) ?? []).map(toDocumentedThemeVariable))
    ).sort();
    ...
    const documentedExclusions = Array.from(styleSettingsSection.matchAll(/^- `(--tps-nn-theme-[\w-]+)`$/gm), match => match[1]);
    ...
    const exampleVariables = Array.from(completeExample.matchAll(/^  (--tps-nn-theme-[\w-]+):/gm), match => match[1]);
    ...
    const coreVariables = coreDeclarations.map(declaration => toDocumentedThemeVariable(declaration.variable));
    assertOrderedMatch(
        coreVariables,
    ...
    const coreDefaults = new Map(
        coreDeclarations.map(declaration => [toDocumentedThemeVariable(declaration.variable), normalizeCssValue(declaration.value)])
    );
    for (const [variable, defaultValue] of coreDefaults) {
        ...
        if (!documentedDefault || normalizeCssValue(toSourceThemeValue(documentedDefault)) !== defaultValue) {
            throw new Error(
    ...
    for (const declaration of parseThemeDeclarations(darkBlock)) {
        const documentedVariable = toDocumentedThemeVariable(declaration.variable);
        const row = referenceRows.get(documentedVariable);
        const documentedValues = Array.from(`${row?.defaultValue ?? ''} ${row?.description ?? ''}`.matchAll(/`([^`]+)`/g), match =>
            normalizeCssValue(toSourceThemeValue(match[1]))
        );
        const darkValue = normalizeCssValue(declaration.value);
        if (!documentedValues.includes(darkValue)) {
            throw new Error(`Theming consistency check failed: ${documentedVariable} dark-mode default ${darkValue} is undocumented`);
        }
    }
    ...
    for (const entry of settings) {
        ...
        const variable = toDocumentedThemeVariable(`--${entry.id}`);
        const documentedDefault = firstCodeValue(referenceRows.get(variable)?.defaultValue ?? '');
        const settingsDefault = `${entry.defaultValue ?? ''}${entry.format ?? ''}`;
        if (toSourceThemeValue(documentedDefault ?? '') !== settingsDefault) {
            throw new Error(
